...
const char *exceptions[32] = {"Divide by zero", "Debug", "NMI", "Breakpoint", "Overflow", "OOB", "Invalid opcode",
                              ...
                              "Unrecognized interrupt", "Coprocessor fault", "Alignment check", "Machine check",
...
void
(*isrHandlers[NUM_ISRS])(struct registers *) = {exceptionHandler, exceptionHandler, exceptionHandler, exceptionHandler,
                                                ...
                                                exceptionHandler, exceptionHandler, exceptionHandler, exceptionHandler,
                                                exceptionHandler, exceptionHandler, exceptionHandler, exceptionHandler,
                                                irqStub, irqStub, irqStub, irqStub, irqStub, irqStub, irqStub, irqStub,
                                                irqStub, irqStub, irqStub, irqStub, irqStub, irqStub, irqStub,
                                                irqStub}; // filling in via for loop kept freezing the system

void handleIsrC(struct registers *regs) {
    if (isrHandlers[regs->int_no]) {
        isrHandlers[regs->int_no](regs);
    }
}
